"""
iter3b.py — Resume from iter3's crash. Task A OOFs already on disk
(lgb, xgb, cat, lgb_ord, lgb_mono, two_stage). We only need to train the
5 Task B models + stage-2 + ensemble + submission.
"""
...
from advanced import (MONO_RATE, monotone_vector_for, multi_target_encode,
                      train_tier4_mixture, train_dart_reg,
                      ridge_blend_classification, ridge_blend_regression)
from iter3 import fit_lgb_reg_mono, fit_xgb_reg_mono, fit_cat_reg, add_multi_te
...
def main():
    ...
    print_header("iter3b — resume from Task B with fixed monotone handling")
    ...
    # --- Task A: load all 6 from disk ---
    print_header("Task A — all 6 OOFs loaded from disk")
    oof_A, test_A = {}, {}
    task_a_models = ["lgb", "xgb", "cat", "lgb_ord", "lgb_mono", "two_stage"]
    for m in task_a_models:
        oof_A[m] = np.load(OOF_DIR / f"{m}_A_oof.npy")
        test_A[m] = np.load(OOF_DIR / f"{m}_A_test.npy")
        acc = accuracy_score(y_tier, oof_A[m].argmax(1))
        print(f"  {m:10s} acc={acc:.4f}")

    # --- Task B: train all fresh ---
    print_header("Task B — fresh training")
    oof_B, test_B = {}, {}

    # LGB with L1 (no monotone — keeps robustness for tier 4)
    t = time.time()
    p_l1 = dict(objective="regression_l1", metric="rmse", learning_rate=0.03,
                num_leaves=95, min_child_samples=20, feature_fraction=0.80,
                bagging_fraction=0.85, bagging_freq=5, lambda_l1=0.1,
                lambda_l2=1.0, verbose=-1, seed=SEED)
    oof = np.zeros(len(X_train))
    tp = np.zeros(len(X_test))
    for tri, vai in folds:
        d_tr = lgb.Dataset(X_train.iloc[tri], y_rate.iloc[tri])
        d_va = lgb.Dataset(X_train.iloc[vai], y_rate.iloc[vai])
        m = lgb.train(p_l1, d_tr, 3500, valid_sets=[d_va],
                      callbacks=[lgb.early_stopping(150), lgb.log_evaluation(0)])
        oof[vai] = m.predict(X_train.iloc[vai], num_iteration=m.best_iteration)
        tp += m.predict(X_test, num_iteration=m.best_iteration) / len(folds)
    oof_B["lgb_l1"] = oof; test_B["lgb_l1"] = tp
    np.save(OOF_DIR / "lgb_l1_B_oof.npy", oof); np.save(OOF_DIR / "lgb_l1_B_test.npy", tp)
    print(f"  lgb_l1 R²={r2_score(y_rate, oof):.4f}  ({time.time()-t:.0f}s)")

    # LGB with L2 + monotone constraints (domain knowledge)
    t = time.time()
    oof_B["lgb_mono"], test_B["lgb_mono"] = fit_lgb_reg_mono(X_train, y_rate, X_test, folds)
    np.save(OOF_DIR / "lgb_mono_B_oof.npy", oof_B["lgb_mono"])
    np.save(OOF_DIR / "lgb_mono_B_test.npy", test_B["lgb_mono"])
    print(f"  lgb_mono R²={r2_score(y_rate, oof_B['lgb_mono']):.4f}  ({time.time()-t:.0f}s)")
    ...
    print(f"  xgb R²={r2_score(y_rate, oof_B['xgb']):.4f}  ({time.time()-t:.0f}s)")

    # CatBoost
    t = time.time()
    oof_B["cat"], test_B["cat"] = fit_cat_reg(X_train, y_rate, X_test, folds, iters=3000)
    np.save(OOF_DIR / "cat_B_oof.npy", oof_B["cat"])
    np.save(OOF_DIR / "cat_B_test.npy", test_B["cat"])
    print(f"  cat R²={r2_score(y_rate, oof_B['cat']):.4f}  ({time.time()-t:.0f}s)")

    # Tier-4 mixture-of-experts
    t = time.time()
    oof_B["mix"], test_B["mix"] = train_tier4_mixture(X_train, y_rate, y_tier,
                                                       X_test, folds, rounds=2500)
    ...
    # --- Ensemble ---
    print_header("Ensemble selection (convex vs Ridge)")
    ...
    print(f"  A convex acc={acc_conv:.4f}  w={ {k: round(float(v),3) for k,v in w_A.items()} }")
    ...
    print_header("Iter 3b — final OOF")
